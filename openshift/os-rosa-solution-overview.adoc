...
* link:https://portal.aws.amazon.com/billing/signup["Account AWS"^]
* link:https://console.redhat.com/["Un account Red Hat"^]
* Utente IAM link:https://www.rosaworkshop.io/rosa/1-account_setup/["con le autorizzazioni appropriate"^] per creare e accedere al cluster ROSA
...
* Helm 3 link:https://docs.aws.amazon.com/eks/latest/userguide/helm.html["documentazione"^]
...
Utilizza il flag node-prep e installa Trident come mostrato. Prima di eseguire il comando di installazione, dovresti aver scaricato il pacchetto di installazione. Fai riferimento a link:https://docs.netapp.com/us-en/trident/trident-install/kubernetes-deploy-tridentctl.html#step-1-download-the-trident-installer-package["Scarica il pacchetto di installazione di Trident"^].
...
La configurazione del back-end di Trident indica a Trident come comunicare con il sistema storage (in questo caso, FSx for ONTAP). Per creare il back-end, forniremo le credenziali della Storage Virtual Machine a cui connettersi, insieme alle interfacce di gestione del cluster e ai dati NFS. Utilizzeremo la link:https://docs.netapp.com/us-en/trident/trident-use/ontap-nas.html["driver ontap-nas"^] per eseguire il provisioning dei volumi di storage nel file system FSx.
...
----
Puoi utilizzare i seguenti comandi per creare un pvc e verificare che sia stato creato.
...
image::redhat-openshift-container-rosa-017.png[tabella, schema, righe postgresql]
