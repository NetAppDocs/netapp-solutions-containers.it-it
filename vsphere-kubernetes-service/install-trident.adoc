...
NOTE: Se i pod di Trident non si avviano a causa di un errore di  `PodSecurity`ammissione, potrebbe essere che le etichette di sicurezza dei pod non siano state applicate allo  `trident`namespace prima dell'installazione. Usa  `--overwrite` per riapplicarle, poi verifica che i pod si avviino.
